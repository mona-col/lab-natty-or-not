# E-book: Como Fazer Renda Extra na Internet - Guia Prático para Mulheres

### Template

```markdown
# E-book: Como Fazer Renda Extra na Internet - Guia Prático para Mulheres

## 📒 Descrição
Este e-book com 6 páginas é um guia prático para mulheres com ideias de como fazer renda estra na internet

## 🤖 Tecnologias Utilizadas
IA Generativa ChatGPT para roteirização;
IA GAMMA para diagramar e criar a arte visual do e-book

## 🧐 Processo de Criação
Utilizei o "ChatGPT" para levantar ideias sobre renda extra na internet para mulheres, estruturando e refinando todo o conteúdo. Depois utilizei o "Leonardo AI" para gerar imagens e ilustrações exclusivas para o  e-book. Para concluir, a montagem final foi feita no Libre Office, por ser uma ferramenta open-source para edição de texto.

## 🚀 Resultados
O e-book apresenta um conteúdo prático com dicas de como as mulheres podem mudar sua realidade financeira realizando renda extra na internet


